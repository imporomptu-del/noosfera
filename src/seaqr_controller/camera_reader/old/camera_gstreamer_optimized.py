...
class GstreamerOptimizedBayer(Node):
    def __init__(self):
        super().__init__('gstreamer_optimized_bayer')
    ...
    def setup_optimized_bayer(self):
        """Setup optimized bayer conversion"""
        try:
            ...
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                ...
                self.get_logger().warn("⚠️ OpenCV CUDA not available, using optimized CPU bayer conversion")
                self.gpu_bayer_available = False
                self.use_gpu_bayer = False
    ...
    def convert_bayer_optimized(self, bayer_data, width, height, bayer_pattern='rggb'):
        """Convert bayer data to RGB using optimized method"""
        if self.use_gpu_bayer:
            return self.convert_bayer_gpu(bayer_data, width, height, bayer_pattern)
        else:
            return self.convert_bayer_cpu_optimized(bayer_data, width, height, bayer_pattern)
    ...
    def convert_bayer_cpu_optimized(self, bayer_data, width, height, bayer_pattern='rggb'):
        """Convert bayer data to RGB using optimized CPU method"""
        try:
            # Reshape bayer data
            bayer_image = bayer_data.reshape((height, width))
            ...
            # Convert on CPU with optimized settings
    ...
    def create_pipeline(self):
        ...
        # Elements - Optimized pipeline with pre-processed RGB data
        ...
        pipeline_type = "GPU-accelerated" if self.use_gpu_bayer else "CPU-optimized"
    ...
    def stop_pipeline(self):
        ...
        self.get_logger().info(f"Average FPS: {self.fps_data['average_fps']:.1f}")
        self.get_logger().info(f"Bayer conversion: {'GPU-accelerated' if self.use_gpu_bayer else 'CPU-optimized'}")
        ...
        def run():
            while self.running:
                try:
                    ...
                    # 2) Convert bayer to RGB (GPU or CPU optimized)
                    bayer_data = np.frombuffer(buf, dtype=np.uint8)
                    rgb_image = self.convert_bayer_optimized(bayer_data, self.width, self.height, self.bayer_pattern)
        ...
        def log_stats():
            while self.running:
                ...
                if self.running:
                    ...
                    bayer_type = "GPU" if self.use_gpu_bayer else "CPU"
                    self.get_logger().info(f"Frame stats ({bayer_type} bayer) - Total: {total}, Dropped: {dropped}, Drop rate: {drop_rate:.2f}%")
