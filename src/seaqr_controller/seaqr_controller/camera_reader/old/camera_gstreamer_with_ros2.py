...
class GstreamerPureGstreamer(Node):
    ...
    def create_pipeline(self):
        ...
        for e in [self.appsrc, convert, nvvidconv, self.encoder, h264parse, splitmux]:
            if not e:
                raise RuntimeError("Failed to create a GStreamer element")

        # Configure appsrc (live + timestamps + raw caps)
        self.appsrc.set_property("is-live", True)
        self.appsrc.set_property("do-timestamp", True)
        self.appsrc.set_property("format", Gst.Format.TIME)
        # Use raw format instead of Bayer (skip Bayer conversion) - higher framerate
        caps_raw = Gst.Caps.from_string(f"video/x-raw,format=GRAY8,width={self.width},height={self.height},framerate=120/1")
        self.appsrc.set_property("caps", caps_raw)
        ...
        # Add + link (simplified pipeline - no Bayer conversion)
        ...
        # Simplified pipeline: appsrc -> convert -> nvvidconv -> encoder -> parse -> splitmux
        if not self.appsrc.link(convert): raise RuntimeError("link appsrc->convert")
        if not convert.link(nvvidconv): raise RuntimeError("link convert->nvvidconv")
        ...
        self.get_logger().info("Pipeline created (appsrc→convert→nvvidconv→enc→parse→splitmuxsink)")    
