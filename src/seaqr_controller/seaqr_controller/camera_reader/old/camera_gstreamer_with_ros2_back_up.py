...
class GstreamerPureGstreamer(Node):
    ...
    def _format_location(self, splitmux, fragment_id: int) -> str:
        ...
        out_dir = "/media/a/E/MyProjects/Water/ros2_ws"
        return os.path.join(out_dir, f"rec_{ts}_{fragment_id:05d}.ts")    
    ...
    def create_pipeline(self):
        ...
        caps_bayer = Gst.Caps.from_string(f"video/x-bayer,format={self.bayer_pattern},width={self.width},height={self.height},framerate=30/1")
        ...
        splitmux.set_property("max-size-time", 5 * 1000 * 1000 * 1000)  # 300s in ns
            ...
            while self.running:
                try:
                    ...
                    if ret != Gst.FlowReturn.OK:
                        self.get_logger().warn(f"appsrc push returned {ret}")
                        time.sleep(0.001)
                        continue
