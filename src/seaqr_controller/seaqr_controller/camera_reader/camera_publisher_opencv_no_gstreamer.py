...
class OpenCVRecorder(Node):
    def __init__(self):
        ...
        # 8) Create GPU pipeline
        self.create_gpu_pipeline()

        # 9) Start recording timer
    ...
    def create_gpu_pipeline(self):
        """Create GPU-accelerated GStreamer pipeline"""
        ...
        pipeline_str = (
            ...
            f"mp4mux ! filesink name=sink"
        )
        
        self.get_logger().info(f"GPU Pipeline: {pipeline_str}")
        ...
        self.running = True
        self.get_logger().info("GPU pipeline started")

    def _detect_gpu_encoder(self):
        """Detect best available GPU encoder"""
        # Try NVIDIA NVENC first
        if self._gst_has_element("nvh264enc"):
            return "nvh264enc preset=llhp bitrate=12000"
        
        # Try NVIDIA Jetson
        if self._gst_has_element("nvv4l2h264enc"):
            return "nvv4l2h264enc bitrate=12000000"
        
        # Try Intel QuickSync
        if self._gst_has_element("vaapih264enc"):
            return "vaapih264enc bitrate=12000"
        
        # Try AMD VCE
        if self._gst_has_element("amfh264enc"):
            return "amfh264enc bitrate=12000"
        
        # Fallback to CPU
        self.get_logger().warn("No GPU encoder found, using CPU x264enc")
        return "x264enc tune=zerolatency bitrate=12000"

    def _gst_has_element(self, element_name):
        """Check if GStreamer element exists"""
        try:
            registry = Gst.Registry.get()
            return registry.find_plugin(element_name) is not None
        except:
            return False

    def start_new_segment(self, frame):
        """Start a new video segment using GPU"""
        ...
        # Create new GPU pipeline with new filename
        self._create_gpu_pipeline_with_file(filename)
        
        self.segment_start_time = time.time()
        self.get_logger().info(f"Started new GPU segment: {filename}")

    def _create_gpu_pipeline_with_file(self, filename):
        """Create GPU pipeline with specific output file"""
        # Detect best available GPU encoder
        encoder = self._detect_gpu_encoder()
        
        # Create pipeline with specific output file
        pipeline_str = (
            f"appsrc name=src is-live=true do-timestamp=true format=time "
            f"caps=video/x-raw,format=BGR,width={self.width},height={self.height},framerate=30/1 ! "
            f"videoconvert ! {encoder} ! "
    ...
    def timer_callback(self):
        ...
        # Push frame to GPU pipeline
        if self.appsrc and self.running:
            try:
                # Convert frame to GStreamer buffer
                buffer = self._frame_to_gst_buffer(frame)
                if buffer:
                    ret = self.appsrc.emit('push-buffer', buffer)
                    if ret != Gst.FlowReturn.OK:
                        self.get_logger().warn("Failed to push buffer to GPU pipeline")
                        self.frame_counters['dropped_frames'] += 1
                        self.frame_counters['current_file_dropped'] += 1
                        return
            except Exception as e:
                self.get_logger().warn(f"GPU pipeline error: {e}")
                self.frame_counters['dropped_frames'] += 1
                self.frame_counters['current_file_dropped'] += 1
                return
    ...
    def _frame_to_gst_buffer(self, frame):
        ...
        try:
            ...
            buffer = Gst.Buffer.new_allocate(None, rgb_frame.nbytes, None)
            buffer.fill(0, rgb_frame.tobytes())
            
            # Set buffer properties
            caps = Gst.Caps.from_string(f"video/x-raw,format=RGB,width={self.width},height={self.height},framerate=30/1")
            buffer.set_caps(caps)
            
            return buffer
        except Exception as e:
            self.get_logger().error(f"Failed to create GStreamer buffer: {e}")
            return None
